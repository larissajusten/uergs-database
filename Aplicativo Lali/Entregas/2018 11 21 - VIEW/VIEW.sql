...
CREATE VIEW view_valor5 AS
	SELECT passageiro.Nome AS "NOME PASSAGEIRO", carona.Horario AS "HORARIO", motorista.Nome AS "NOME MOTORISTA"
	FROM carona
	INNER JOIN motorista on (motorista.idMotorista = carona.idMotorista)
	INNER JOIN solicitar_carona on (carona.idCarona = solicitar_carona.idCarona)
	INNER JOIN passageiro on (solicitar_carona.idPassageiro = passageiro.idPassageiro)
...
CREATE VIEW info_caronas AS
	SELECT carona.idCarona AS "Id", motorista.Nome AS "motorista", carona.Data AS "data", carona.Horario AS "hora", carona.Valor AS "valor", carona.Numero_Vagas AS "vagas", carona.Veiculo AS "veiculo", carona.Partida AS "partida", carona.Chegada AS "chegada", locais_de_parada.Descricao AS "locais"
	FROM carona
	INNER JOIN motorista ON motorista.idMotorista = carona.idMotorista
